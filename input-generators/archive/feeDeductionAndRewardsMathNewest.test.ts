...
describe('Fee Deduction and Rewards Math Validation (Newest)', () => {
    test('should match all validator calculations with MAX_SSV_REWARD applied first, then fee deductions', () => {
        process.stdout.write('\n🧮 Starting Fee Deduction and Rewards Math Validation (Newest)...\n\n');
        ...
        let totalBaseRewards = new Decimal(0);
        ...
        let totalFinalRewards = new Decimal(0);
        let totalNetworkFees = new Decimal(0);
        ...
        for (const { validator, baseReward } of validatorBaseRewards) {
            ...
            const finalRewardHighPrecision = Decimal.max(0, cappedReward.sub(calculatedFeeDeduction));
        ...
        if (rewardMismatches === 0 && feeMismatches === 0) {
            ...
            process.stdout.write(`✅ All ${totalValidators} validators match to at least 14th decimal precision\n`);
        }
    });
});