...
describe('Fee Deduction and Rewards Math Validation', () => {
    test('should match all validator calculations with proper APR boost detection', () => {
        ...
        const maxSsvReward = new Decimal(process.env.MAX_SSV_REWARD!);
        ...
        // First pass: Calculate all rewards without comparison to determine if ratio is needed
        process.stdout.write(`🔄 First pass: Calculating all rewards to check against MAX_SSV_REWARD...\n`);
        let totalCalculatedRewards = new Decimal(0);
        const validatorRewards: Array<{ validator: ValidatorData, calculatedReward: any, calculatedFeeDeduction: any }> = [];
        ...
        for (const validator of validators) {
            ...
            totalCalculatedRewards = totalCalculatedRewards.add(calculatedReward);
        ...
        // Check if total rewards exceed MAX_SSV_REWARD and calculate ratio if needed
        let rewardRatio = new Decimal(1);
        if (totalCalculatedRewards.gt(maxSsvReward)) {
            rewardRatio = maxSsvReward.div(totalCalculatedRewards);
            process.stdout.write(`🚨 Total calculated rewards (${totalCalculatedRewards.toFixed(18)}) exceed MAX_SSV_REWARD (${maxSsvReward.toString()})\n`);
            process.stdout.write(`📊 Applying ratio: ${rewardRatio.toFixed(18)} (${rewardRatio.mul(100).toFixed(4)}%)\n`);
            
            // Calculate what the ratio-adjusted total should be
            const adjustedTotalRewards = totalCalculatedRewards.mul(rewardRatio);
            process.stdout.write(`📊 Ratio-adjusted total rewards: ${adjustedTotalRewards.toFixed(18)}\n\n`);
        } else {
            process.stdout.write(`✅ Total calculated rewards (${totalCalculatedRewards.toFixed(18)}) within MAX_SSV_REWARD limit (${maxSsvReward.toString()})\n`);
            process.stdout.write(`📊 No ratio adjustment needed\n\n`);
        }
        
        // Also calculate the total from the CSV for comparison
        let totalExpectedRewards = new Decimal(0);
        for (const validator of validators) {
            totalExpectedRewards = totalExpectedRewards.add(new Decimal(validator.Reward));
        }
        ...
        const fifteenthDecimalFailures: Array<{
            publicKey: string;
        ...
        // Validate each validator using pre-calculated values with ratio adjustment
        for (let i = 0; i < validatorRewards.length; i++) {
            const { validator, calculatedReward: baseCalculatedReward, calculatedFeeDeduction } = validatorRewards[i];
            totalValidators++;
            
            // Apply ratio to reward (fee deduction remains unchanged)
            const calculatedReward = baseCalculatedReward.mul(rewardRatio);
        ...
        if (fifteenthDecimalFailures.length > 0) {
            process.stdout.write(`❌ VALIDATORS THAT FAILED AT 15th DECIMAL (${fifteenthDecimalFailures.length} total, showing first 10):\n\n`);
            fifteenthDecimalFailures.slice(0, 10).forEach((failure, index) => {
