...
func (validatorEventL) LoadContractEvent(ctx context.Context, e boil.ContextExecutor, singular bool, maybeValidatorEvent interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		args = append(args, object.ContractEventID)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.ContractEventID {
					continue Outer
				}
			}

			args = append(args, obj.ContractEventID)
	...
	query := NewQuery(
		...
		qm.WhereIn(`contract_events.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *ValidatorEvent) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"validator_events\"", updateOnConflict, ret, update, conflict, insert)
